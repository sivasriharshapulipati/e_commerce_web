...
const Sidebar = () => {
  ...
  >
    ...
    <div className=' flex flex-col gap-y-2 h-[320px] lg:h-[400px] overflow-y-auto overflow-x-hidden border-b'>{cart.map((item) => {
      return <CartItem item={item} key={item.id} />;
    })}
    </div>
    <div className=' flex flex-col gap-y-2 border-2 mt-4'>
      ...
      <Link to='/' className='bg-primary flex p-2 justify-center items-center text-white w-full font-medium'>Check out</Link>
    </div>
  </div>
  );
};
